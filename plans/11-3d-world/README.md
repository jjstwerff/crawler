...
| **P6b** — parallax layers: cache the air box, re-project it | M | re-render counts + worst per-frame parallax error; a pop-free walk | **CRITERION DONE, and it answered the phase**: `parallax.loft` + `parallaxtest` (pure camera math, no GL). ⚠ **The compression is a term DESIGN.md's table omits — every cache lifetime there is exactly 10x too long** — and priced correctly the cache **buys ONE layer**: layers at 1.0 and 4.0 km have budgets under one hex step and are not cacheable at all, so they must be GEOMETRY, which is what P6's skirt already is. Building the layer machinery is therefore worth 1 of 4 layers; **decide before building it**. ⚠ Also: nine of twelve bearings are fully occluded by the near rim from the shipped start |
