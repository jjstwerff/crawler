...
| C24 | **store-pressure threshold SIGSEGV:** growing the program (Sim +13 fields, +modules, even **never-executed call sites**) flips `quickslottest`'s `sim_descend` into a SIGSEGV (`op=221`, `fn (?)`) — multiple toggles that change only ALLOCATION VOLUME flip it on/off (a 5-line probe crashed until the rc_* cache refactor; quickslottest still crashes with AND without the castfx wiring, clean cache). Refuted: the >1024-fn / pc>2^17 encoding theory (standalone 1100-fn program runs correctly). **BLOCKS the class-track commit** (gate red at [quickslot]); likely the vector-lifetime bug already in active development. | HIGH | **filed loft#306** (sev:high, wa:none, area:store-lifetime+runtime). #303/#290 family. Re-run recipe in the issue; crawler's class worktree is the regression case. |
