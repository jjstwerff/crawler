...
| C24 | **store-pressure threshold SIGSEGV:** growing the program (Sim +13 fields, +modules, even **never-executed call sites**) flips `quickslottest`'s `sim_descend` into a SIGSEGV — multiple toggles that change only ALLOCATION VOLUME flip it on/off. **DIAGNOSED (2026-06-10): op 221 = `copy_record`** — the by-value Sim pass into `sim_descend` faults copying the struct, i.e. a field references a freed/corrupt slot after the use_slot traffic and the full-struct copy is the first guaranteed dereference (#303's wrong-results = lighter touches of the same corruption). `LOFT_UAF=1` stays silent (interior struct fields may be outside its frame-variable scan). Refuted standalone: >1024 fns AND >2^17 bytecode ops both run correctly; 5th minimization attempt (text-ring + big vectors + by-value copies + copy_record) also correct — needs the real Sim's allocation history. **BLOCKS the class-track gate** (red at [quickslot]). | HIGH | **loft#306 OPEN** (sev:high, wa:none). **Re-verified still crashing on loft2 `a9edb27c`** (post-#303-fix, fresh build, clean cache, same signature); full diagnostics posted as an issue comment. Gate = 31/32 meanwhile. |
