...
>
> **Full audit 2026-06-10** (the post-#306 store fixes, probe-verified on the installed
> loft): **C1/C3/C4/C7/C10/C13 fixed**; **C2 covered** via its fixed instances; **C6
> by-design** (loft C69); the two SURVIVORS distilled to minimal repros and filed —
> **#319** (struct-literal comprehensions panic codegen) + **#320** (capture-append-
> reassign empties a struct's vector field). Gate 33/33. Bare `loft` = the INSTALLED
> binary (a `--path` swaps only the stdlib) — verify fixes via `make test LOFT_REPO=`.
>
...
| ID  | Title | Sev | Status |
|-----|-------|-----|--------|
| C1  | `vector<struct>[i] ?? structfn()` SIGSEGV | HIGH | **FIXED — verified 2026-06-10** (probe passes on the installed post-#306 loft; the store fixes covered it) |
| C2  | `println` changes results (Heisenbug) | HIGH | **covered by the #303/#306 fixes** (its concrete instances C22/C24 verified fixed; never had an independent repro — WATCH) |
| C3  | struct copy drops a `text` field | HIGH | **FIXED — verified 2026-06-10** (copy-chain probe keeps the text) |
| C4  | cross-module `&Struct` mutation lost | HIGH | **FIXED — verified 2026-06-10** (two-module probe mutates correctly). UNBLOCKS a shared seeded-RNG lib; the same-module idiom stays as defense until soaked |
| C5  | comprehension of STRUCT literals panics codegen (`codegen.rs:2864 Incorrect var __ncc_N[65535]`); integer comprehensions fine | HIGH | **filed loft#319** (now standalone-minimal, 5 lines — distilled into a clean panic after the store fixes). wa:clean — explicit loop+append |
| C6  | `!` on an integer is not logical-not | MED | **#253 CLOSED BY-DESIGN** (loft C69: `!x` on a non-boolean = a NULL test; always-false warning added for `not null` operands). Survival-guide note stays |
| C7  | chained `as` casts don't parse | LOW | **#254 FIXED — verified 2026-06-10** (`3.7 as integer as float` = 3) |
| C8  | `gl_load_font` relative-path base | LOW | **#255 CLOSED** (source-relative asset loading); behavior unverified locally — the view keeps its candidate list until a `make play` confirms |
| C9  | native: `return let mut …` invalid Rust | HIGH | **FIXED** (re-verified — compiles native) |
| C10 | boolean `?? fallback` unreliable | MED | **#256 FIXED — verified 2026-06-10** (`false ?? true` = false) |
...
| C13 | copy `&ref` to local panics codegen | HIGH | **#257 FIXED — verified 2026-06-10** (mutating-fn copy works; never-mutated `&` now a clean diagnostic) |
...
| C18 | runtime append to a struct-field vector: direct `s.v += [x]` **now WORKS**, but capture-append-reassign **EMPTIES the field** (silent loss, worse than recorded) | HIGH | **filed loft#320** (now standalone-minimal, 6 lines). wa:clean — direct append or the index-write idiom (which stays the survival-guide default) |
...
| C20 | auto-native cdylib crate name keeps the package version's dots (`loft_auto_glb-0.1.0` → invalid) → a **used** auto-native lib's cdylib silently fails to build → its native fns vanish (`load_all()` "native function not loaded") | MED | **loft#294 — FIXED by c494e0a** (loft2: `auto_cdylib_stem()` maps every non-`[A-Za-z0-9_]` char to `_`). This WAS the graphics-load blocker (not distinct after all — the glb-only workaround just missed the real failing lib). **Verified:** graphics restored + the `@` glyph renders (the font fix is confirmed). |
| C21 | a live local/field reads garbage mid-loop → `long overflow: N * <float-bitpattern>` (varies per run). Repro: `sim_compute_fov` on a 41×41 `sim_new_gen` dungeon sim under `--interpret` — `s.lw` reads ~4.6e18 mid-loop though it's 41 by value; 21×21 arena does NOT trip it. ROOT CAUSE (per the fix): a **parser** bug — a thin **struct/tuple-return**'s hidden result arg was dropped (`collect_hidden_ref_args` didn't unwrap the `Span`), so per-cell tuple-returns like `hex_to_px` corrupted the frame. (I'd guessed store-lifetime; symptom only.) | HIGH | **loft#299 — FIXED by loft2 `fc80115`** (`fix(parser): unwrap Span in collect_hidden_ref_args`). **VERIFIED:** rebuilt `../loft2/target/release/loft`, repro `src/fovbug.loft` survives 5/5, full gate 24/24 (`make test LOFT_REPO=../loft2`, incl. QUEST OK). **Pending `make install` of loft2** so the default `loft`/`make play` get it. |
| C22 | holding several large structs live at once + a **deferred** boolean comparing a field across two of them → **wrong result** (seed-distinct values compare equal); a `println` reading the fields first **flips it to correct** (the C2 Heisenbug, fresh instance). Repro: `unknowntest` holding ~7 `sim_new_gen` Sims → `diff_seed=false` though seed-7 ≠ seed-8; couldn't minimise standalone (small struct / 1681-pad / by-value text copy / 200-alloc churn all stayed correct). **WORKAROUND (kept):** consume each `sim_new_gen(...)` straight into a `text`/`bool` — never hold many big Sims. | HIGH | **loft#303 — FIXED by loft2 `5d5488d5`** (unify the shared-bridge marshallability judgment). **VERIFIED 2026-06-10:** the original held-7-Sims shape re-run 3/3 → `diff_seed=true` on loft2 `a9edb27c`. The defensive consume-immediately discipline stays (C24 shows the family isn't fully closed). |
| C23 (#305 CLOSED upstream; local native verify masked by the E0514 rustc-mismatch env — unique-name workaround stays, harmless) | **backend divergence:** two modules each exporting a pub fn with the **same name** (called module-qualified) run fine under `--interpret` but the **native/WASM** codegen emits a duplicate global symbol → `E0428: loft_shared_n_<fn> defined multiple times`. The symbol = `loft_shared_n_<fn_name>` (NOT module-namespaced), visible in `native-auto/loft_auto_*.rs`. Hit by the race bundles (all exported `race_defs()`). **Lesson: interpret-only verification misses it — check the compile path.** **WORKAROUND:** globally-unique pub fn names per bundle (`<key>_race_defs()`). | MED | **filed loft#305** (sev:medium, wa:clean, area:native+codegen). Note: world bundles' fixed `monster_defs()`/`item_defs()` have the same latent clash once a 2nd world bundle adds that section. |
| C24 | **store-pressure threshold SIGSEGV:** growing the program (Sim +13 fields, +modules, even **never-executed call sites**) flipped `quickslottest`'s `sim_descend` into a SIGSEGV. **DIAGNOSED: op 221 = `copy_record`** — a Sim field carried a freed/dangling store-slot id after use_slot traffic; the full-struct copy (or the old Sim's `free_ref` — the `allocation.rs:179` OOB-panic variant in the live-game flow) was the first dereference. ROOT (per the fix): **a returned VIEW let the caller free a store it didn't own** → premature free → the dangling id. #303's wrong-results = lighter touches of the same corruption. | HIGH | **loft#306 — FIXED by loft2 `bug123` @ `913b625b`** (returned views must not let callers free stores they don't own). **VERIFIED 2026-06-10:** quickslottest 3/3 OK, the game-flow probe 3/3 SURVIVED, the C22 shape 3/3 correct, **full gate 32/32** (first complete pass; the class track is UNBLOCKED). **INSTALLED** the same day (`make install` from bug123; loft-doctor all green, `make test` passes with the DEFAULT toolchain — no LOFT_REPO needed). **Tooling lesson: bare `loft` = the INSTALLED binary** — `--path ../loft2/` swaps only the stdlib; verify fixes with `$(LOFT_REPO)/target/release/loft` or `make test LOFT_REPO=`. |
| C25 | **doc/impl divergence:** PACKAGES.md says compile-time `use` resolution consults `[dependencies]` `{ path = ... }` entries (step 2) — observed: it does NOT (a path-dep consumer fails "Library not found"; only `loft test --deps` honours path deps). Working routes verified: `--lib` dirs, SIBLING packages, `loft install .`, the registry. | LOW | **to file** (doc bug or missing resolver feature — owner's call which); EXTRACTION.md plans around it (`--lib` dev route) |
| C26 | **stale program-cache on lib edits:** `~/.cache/loft/program-*` is not invalidated when a `--lib`-resolved dependency file changes — the OLD bytecode keeps executing (even its overflow trap fired from deleted code). Local `.loft` dirs are NOT the culprit. Poisons lib-iteration debugging silently. **WORKAROUND:** `rm ~/.cache/loft/program-*` after lib edits. | MED | **filed loft#322** (sev:medium, wa:clean, area:packages). Found probing the RandStream methods-way API |
