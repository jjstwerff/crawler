...
| K3 live-reload dev mode | S | BLOCKED on two upstream gates found by the smoke (2026-06-12): loft#346 (the shadow session resolves the stdlib as relative `default` — symlink workaround verified: `live-reload: watching`) and loft#347 (the shadow parse counts crawler's 35 standing sim.loft WARNINGS as disqualifying). Either #347 lands or crawler zeroes its warnings; the smoke (`/tmp` recipe in the issues: live-edit view_draw's wall color, pixel-check the redraw) is ready to rerun |
| K4 MP lockstep | L | gated on a design pass (K1/K2 shipped the foundations) |
| K5 "next world" — in-game bundle reload | M | **SCANNER PORTED 2026-06-12** — `tools/gen_bundles.loft` (stdlib only; root from $PWD since relative paths resolve script-side), verified BYTE-IDENTICAL to the python generator across all seven registries; python DELETED, Makefile + gate call loft. En route: loft#348 filed (sequential `.lines()` reads return empty — `content()+split_text` is the clean workaround). Remaining: the in-game action: re-scan → regenerate registries → `rebuild_start()` (background, play continues) → poll `rebuild_status()` in on_tick → `swap_start()` at the world boundary (the ~35 ms window blink = the loading beat). Rides the BUILD-SWAP tier (S4/S5/S8) — NOT per-fn live-reload, so loft#346/#347 don't block it; story's loop already runs `kernel_swap_step()` since K1. Caveat: the handoff must be KEY-based (catalogs reshuffle — same rule as the G4 save format). v1 = fresh character, new world, new bundles |
