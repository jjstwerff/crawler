...
def main():
    ...
    # L6 — the engine must not name a BUNDLE's item by key. The existing seam grep in
    # run_tests.sh covers monsters (`mon_find("`); this is the item half. A generated file
    # naming bundle content is its whole job, so generated files are exempt BY THEIR OWN
    # header rather than by a filename pattern anyone can forget to match.
    bundle_items = {}
    for pat in ("bundles/**/*.loft", "bundles/**/*.json"):
        for f in ROOT.glob(pat):
            if not f.is_file():
                continue
            t = read(f)
            for k in re.findall(r'i_key:\s*"([^"]+)"', t):
                bundle_items.setdefault(k, f.relative_to(ROOT).as_posix())
    engine_items = set(re.findall(r'i_key:\s*"([^"]+)"', read(ROOT / "src" / "items.loft")))
    bundle_only = {k: v for k, v in bundle_items.items() if k not in engine_items}
    for f in sorted((ROOT / "src").glob("*.loft")):
        # Tests are exempt and that is not a loophole: a gate's job is to assert that
        # SPECIFIC content behaves ("the desert_surprise antivenin cures venom"), so it
        # must name it. The rule is about the engine reaching content at RUNTIME.
        if not f.is_file() or f.stem.endswith("test"):
            continue
        text = read(f)
        if "GENERATED by" in text[:400]:
            continue
        for k, owner in sorted(bundle_only.items()):
            if f'"{k}"' in text:
                report("L6", f"{f.stem}:{k}",
                    f"L6  src/{f.name} names `{k}`, an item defined by a BUNDLE ({owner}).\n"
                    f"    The engine must not reach content by key — that is what stops a\n"
                    f"    stranger's bundle needing an engine edit (BUNDLE.md standing check).\n"
                    f"    Ship the routine with the item: bundles/<b>/items/{k}.loft `pub fn\n"
