...
- **Local events / conditions — the *shared ground* every person is tied into
  ...
  - **Mobility — *no actor is totally static*; pushed hard/long enough, any actor
    relocates (slowly).** A web-node has a **location**, and **relocation is itself a
    reaction**: sustained pressure (from the coupled activity/conditions) eventually
    **moves** the node — *gradually*, on the world/strategic timescale, never a jump.
    Across every domain: **spirits migrate** (moros's chaos spirits drifting out of
    the blasted lands → the spreading desert; a fire elemental leaving for the
    mountains; the *moved* shackle towers), **economies relocate** (trade re-routes,
    markets and boom-towns shift as resources deplete), **factions' territory shifts**,
    **people displace** (refugees), and **even cities relocate** (Steadington consumed
    → its people move; Laurent's *flying* city — relocation as a setpiece). So **the
    map is *dynamic* over a campaign**: the history-gen produces *past* migrations (a
    founding, an abandonment, a drift); play produces *ongoing* ones. The player can
    **push** an actor (drive a spirit out, force a retreat) or **resist** (hold a
